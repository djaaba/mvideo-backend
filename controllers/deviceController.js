...
class DeviceController {
    ...
    async getBestsellers(req, res) {
        let devices = await Device.findAll({ order: [['purchasesCount', 'DESC']], limit: 8 })

        return res.json(devices)
    }

    async getMostViewed(req, res) {
        let devices = await Device.findAll({ order: [['viewCount', 'DESC']], limit: 8 })

        return res.json(devices)
    }

    async getDiscounted(req, res) {
        let devices = await Device.findAll({ order: [['discount', 'DESC']], limit: 8 })
